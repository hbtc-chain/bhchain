...
import (
	...
	authsim "github.com/hbtc-chain/bhchain/x/custodianunit/simulation"
	distr "github.com/hbtc-chain/bhchain/x/distribution"
	distrsim "github.com/hbtc-chain/bhchain/x/distribution/simulation"
	"github.com/hbtc-chain/bhchain/x/gov"
	govsim "github.com/hbtc-chain/bhchain/x/gov/simulation"
	"github.com/hbtc-chain/bhchain/x/mint"
	"github.com/hbtc-chain/bhchain/x/params"
	paramsim "github.com/hbtc-chain/bhchain/x/params/simulation"
	"github.com/hbtc-chain/bhchain/x/simulation"
	"github.com/hbtc-chain/bhchain/x/slashing"
	slashingsim "github.com/hbtc-chain/bhchain/x/slashing/simulation"
	"github.com/hbtc-chain/bhchain/x/staking"
	stakingsim "github.com/hbtc-chain/bhchain/x/staking/simulation"
...
func getSimulateFromSeedInput(tb testing.TB, w io.Writer, app *bhexapp) (
...
func appStateFn(
	r *rand.Rand, accs []simulation.CU,
) (appState json.RawMessage, simAccs []simulation.CU, chainID string, genesisTimestamp time.Time) {
...
func appStateRandomizedFn(
	r *rand.Rand, accs []simulation.CU, genesisTimestamp time.Time, appParams simulation.AppParams,
) (json.RawMessage, []simulation.CU, string) {
...
func testAndRunTxs(app *bhexapp) []simulation.WeightedOperation {
	cdc := MakeCodec()
	...
	return []simulation.WeightedOperation{
		{
			...
			authsim.SimulateDeductFee(app.cuKeeper, app.supplyKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
			...
			transfer.SimulateMsgSend(app.cuKeeper, app.transferKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
			...
			transfer.SimulateSingleInputMsgMultiSend(app.cuKeeper, app.transferKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
			...
			distrsim.SimulateMsgSetWithdrawAddress(app.cuKeeper, app.distrKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
			...
			distrsim.SimulateMsgWithdrawDelegatorReward(app.cuKeeper, app.distrKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
			...
			distrsim.SimulateMsgWithdrawValidatorCommission(app.cuKeeper, app.distrKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
		...
		},
		{
			...
			stakingsim.SimulateMsgCreateValidator(app.cuKeeper, app.stakingKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
		...
		},
		{
			...
			stakingsim.SimulateMsgDelegate(app.cuKeeper, app.stakingKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
			...
			stakingsim.SimulateMsgUndelegate(app.cuKeeper, app.stakingKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
			...
			stakingsim.SimulateMsgBeginRedelegate(app.cuKeeper, app.stakingKeeper),
		},
		{
			func(_ *rand.Rand) int {
				var v int
...
func invariants(app *bhexapp) []sdk.Invariant {
	// TODO: fix PeriodicInvariants, it doesn't seem to call individual invariants for a period of 1
	// Ref: https://github.com/hbtc-chain/bhchain/issues/4631
	if period == 1 {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/hbtc-chain/bhchain/bhexapp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	app := Newbhexapp(logger, db, nil, true, 0, nil, nil, "")
...
func TestFullAppSimulation(t *testing.T) {
	...
	app := Newbhexapp(logger, db, nil, true, 0, nil, nil, "", fauxMerkleModeOpt)
	require.Equal(t, "bhexapp", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := Newbhexapp(log.NewNopLogger(), newDB, nil, true, 0, nil, nil, "", fauxMerkleModeOpt)
	require.Equal(t, "bhexapp", newApp.Name())
	...
	storeKeysPrefixes := []StoreKeysPrefixes{
		...
		{app.keys[custodianunit.StoreKey], newApp.keys[custodianunit.StoreKey], [][]byte{}},
