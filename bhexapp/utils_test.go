...
func TestGetSimulationLog(t *testing.T) {
	...
	tests := []struct {
		...
		{custodianunit.StoreKey, cmn.KVPair{Key: custodianunit.AddressStoreKey(delAddr1), Value: cdc.MustMarshalBinaryBare(custodianunit.BaseCU{})}},
...
//	cdc := makeTestCodec()
//	acc := custodianunit.NewBaseCUWithAddress(delAddr1, sdk.CUTypeUser)
//	globalAccNumber := uint64(10)
//
//	kvPairs := cmn.KVPairs{
//		cmn.KVPair{Key: custodianunit.AddressStoreKey(delAddr1), Value: cdc.MustMarshalBinaryBare(acc)},
//		cmn.KVPair{Key: custodianunit.GlobalCUNumberKey, Value: cdc.MustMarshalBinaryLengthPrefixed(globalAccNumber)},
...
func TestDecodeStakingStore(t *testing.T) {
	...
	val := staking.NewValidator(valAddr1, delPk1, staking.NewDescription("test", "test", "test", "test"), true)
...
func TestDecodeGovStore(t *testing.T) {
	...
	proposal := gov.NewProposal(content, 1, endTime, endTime.Add(24*time.Hour), 0)
