...
type Keybase interface {
	...
	GetByAddress(address types.CUAddress) (Info, error)
	...
	// CreateAccount creates an CU based using the BIP44 path (44'/496'/{CU}'/0/{index}
	CreateAccount(name, mnemonic, bip39Passwd, encryptPasswd string, CU uint32, index uint32) (Info, error)
	...
	// See https://github.com/hbtc-chain/bhchain/issues/2095
	...
	CreateLedger(name string, algo SigningAlgo, hrp string, CU, index uint32) (info Info, err error)
...
// GetType implements Info interface
func (i localInfo) GetAddress() types.CUAddress {
...
func (i ledgerInfo) GetAddress() types.CUAddress {
	return i.PubKey.Address().Bytes()
}
...
// GetAddress implements Info interface
func (i offlineInfo) GetAddress() types.CUAddress {
	return i.PubKey.Address().Bytes()
}
...
// GetAddress implements Info interface
func (i multiInfo) GetAddress() types.CUAddress {
	return i.PubKey.Address().Bytes()
}
