...
func (lkb lazyKeybase) GetByAddress(address sdk.CUAddress) (Info, error) {
	db, err := sdk.NewLevelDB(lkb.name, lkb.dir)
	if err != nil {
		return nil, err
	}
...
func (lkb lazyKeybase) CreateAccount(name, mnemonic, bip39Passwd, encryptPasswd string, CU uint32, index uint32) (Info, error) {
	db, err := sdk.NewLevelDB(lkb.name, lkb.dir)
	if err != nil {
		return nil, err
	}
	...
	return NewDbKeybase(db).CreateAccount(name, mnemonic, bip39Passwd, encryptPasswd, CU, index)
}
...
func (lkb lazyKeybase) CreateLedger(name string, algo SigningAlgo, hrp string, CU, index uint32) (info Info, err error) {
	db, err := sdk.NewLevelDB(lkb.name, lkb.dir)
	if err != nil {
		return nil, err
	}
	...
	return NewDbKeybase(db).CreateLedger(name, algo, hrp, CU, index)
}
