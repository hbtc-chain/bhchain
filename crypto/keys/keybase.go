...
import (
	...
	"github.com/hbtc-chain/bhchain/crypto"
	"github.com/hbtc-chain/bhchain/crypto/keys/hd"
	"github.com/hbtc-chain/bhchain/crypto/keys/keyerror"
	"github.com/hbtc-chain/bhchain/crypto/keys/mintkey"
...
func (kb dbKeybase) CreateAccount(name, mnemonic, bip39Passwd, encryptPasswd string, CU uint32, index uint32) (Info, error) {
	coinType := types.GetConfig().GetCoinType()
	hdPath := hd.NewFundraiserParams(CU, coinType, index)
...
func (kb dbKeybase) CreateLedger(name string, algo SigningAlgo, hrp string, CU, index uint32) (Info, error) {
	if algo != Secp256k1 {
...
func (kb dbKeybase) GetByAddress(address types.CUAddress) (Info, error) {
...
func addrKey(address types.CUAddress) []byte {
