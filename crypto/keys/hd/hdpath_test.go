...
func TestStringifyFundraiserPathParams(t *testing.T) {
	...
	require.Equal(t, "44'/496'/4'/0/22", path.String())

	path = NewFundraiserParams(4, types.CoinType, 57)
	require.Equal(t, "44'/496'/4'/0/57", path.String())
...
func TestPathToArray(t *testing.T) {
	path := NewParams(44, 496, 1, false, 4)
	require.Equal(t, "[44 496 1 0 4]", fmt.Sprintf("%v", path.DerivationPath()))

	path = NewParams(44, 496, 2, true, 15)
	require.Equal(t, "[44 496 2 1 15]", fmt.Sprintf("%v", path.DerivationPath()))
}
...
func TestParamsFromPath(t *testing.T) {
	goodCases := []struct {
		...
		{&BIP44Params{44, 496, 52, true, 41}, "44'/496'/52'/1/41"},
	}
