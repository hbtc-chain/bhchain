...
and a bip32 HD path to derive a specific CU. The key will be stored under the given name
...
`,
	...
	cmd.Flags().Uint32(flagAccount, 0, "CU number for HD derivation")
...
func runAddCmd(cmd *cobra.Command, args []string) error {
	...
	if viper.GetBool(flagDryRun) {
		...
		_, err = kb.Get(name)
		if err == nil {
			// CU exists, ask for user confirmation
	...
	CU := uint32(viper.GetInt(flagAccount))
	index := uint32(viper.GetInt(flagIndex))

	// If we're using ledger, only thing we need is the path and the bech32 prefix.
	if viper.GetBool(flags.FlagUseLedger) {
		bech32PrefixAccAddr := sdk.GetConfig().GetBech32AccountAddrPrefix()
		info, err := kb.CreateLedger(name, keys.Secp256k1, bech32PrefixAccAddr, CU, index)
		if err != nil {
			return err
		}
	...
	info, err := kb.CreateAccount(name, mnemonic, bip39Passphrase, encryptPassword, CU, index)
	if err != nil {
		return err
	}
...
func printCreate(cmd *cobra.Command, info keys.Info, showMnemonic bool, mnemonic string) error {
	...
	case OutputFormatText:
		...
		if showMnemonic {
			...
			cmd.PrintErrln("It is the only way to recover your CU if you ever forget your password.")
			cmd.PrintErrln("")
