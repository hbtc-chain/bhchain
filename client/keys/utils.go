...
// NewDirectKeyBase return non-lazy keybase
func NewDirectKeyBase() (keys.Keybase, error) {
	rootDir := viper.GetString(flags.FlagHome)
	db, err := sdk.NewLevelDB(defaultKeyDBName, filepath.Join(rootDir, "keys"))
	if err != nil {
		return nil, err
	}

	return keys.NewDbKeybase(db), nil
}
