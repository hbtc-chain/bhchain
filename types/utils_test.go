...
func TestEstimateSignedUtxoTxSize(t *testing.T) {
	cases := []struct {
		numVin       int
		numVout      int
		expectedSize int
	...
	for _, tc := range cases {
		size := EstimateSignedUtxoTxSize(tc.numVin, tc.numVout)
		require.Equal(t, tc.expectedSize, int(size.Int64()))
	}
}

// maj23 >2/3
func TestMajority23(t *testing.T) {
	cases := []struct {
		num   int
		maj23 int //major > num*2/3
	}{
		{4, 3},
		{5, 4},
		{6, 5},
	...
	for _, tc := range cases {
		require.Equal(t, tc.maj23, Majority23(tc.num))
	}
}
...
func TestStringsIndex(t *testing.T) {
	cases := []struct {
		...
		{s: []string{"aaa", "bbb", "ccc"}, search: "ccc", expect: 2},
		{s: []string{"aaa", "bbb", "ccc"}, search: "aaa", expect: 0},
		{s: []string{"aaa", "bbb", "ccc"}, search: "ddd", expect: -1},
	}

	for _, c := range cases {
		got := StringsIndex(c.s, c.search)
