...
func TestBaseReqValidateBasic(t *testing.T) {
	fromAddr := "HBCLmQcskpdQivEkRrh1gNPm7c9aVB8hh1fy"
	tenstakes, err := sdk.ParseCoins("10stake")
	require.NoError(t, err)
	onestake, err := sdk.ParseDecCoins("1.0stake")
	require.NoError(t, err)
	...
	req5 := NewBaseReq(
		fromAddr, "", "nonempty", "", "", 0, 0, sdk.Coins{}, sdk.DecCoins{}, false,
	)
...
func TestProcessPostResponse(t *testing.T) {
	...
	type mockAccount struct {
		Address  sdk.CUAddress `json:"address"`
		Coins    sdk.Coins     `json:"coins"`
		PubKey   crypto.PubKey `json:"public_key"`
		CUNumber uint64        `json:"cu_number"`
	...
	coins := sdk.NewCoins(sdk.NewCoin("atom", sdk.NewInt(100)), sdk.NewCoin("tree", sdk.NewInt(125)))
	...
	cdc.RegisterConcrete(&mockAccount{}, "hbtcchain/mockAccount", nil)
