...
import (
	...
	"github.com/hbtc-chain/bhchain/store/gaskv"
	stypes "github.com/hbtc-chain/bhchain/store/types"
)
...
type BasicSettleQuota struct {
	limit    Gas
	consumed map[string]Gas
}

func NewBasicSettleQuota(limit Gas) *BasicSettleQuota {
	return &BasicSettleQuota{
		limit:    limit,
		consumed: make(map[string]Gas),
	}
}

func (b *BasicSettleQuota) Consume(valAddress ValAddress, current Gas) bool {
	var after Gas
	key := string(valAddress)
	if before, ok := b.consumed[key]; ok {
		after = before + current
	} else {
		after = current
	}
	if after <= b.limit {
		b.consumed[key] = after
		return true
	} else {
		return false
	}
