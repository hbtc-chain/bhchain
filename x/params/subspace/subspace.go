...
// Get to ParamSet
func (s Subspace) GetParamSetWithSubkey(ctx sdk.Context, subkey []byte, ps ParamSet) {
	for _, pair := range ps.ParamSetPairs() {
		s.GetWithSubkey(ctx, pair.Key, subkey, pair.Value)
	}
}

// Set from ParamSet
func (s Subspace) SetParamSetWithSubkey(ctx sdk.Context, subkey []byte, ps ParamSet) {
	for _, pair := range ps.ParamSetPairs() {
		v := reflect.Indirect(reflect.ValueOf(pair.Value)).Interface()
		s.SetWithSubkey(ctx, pair.Key, subkey, v)
	}
}
