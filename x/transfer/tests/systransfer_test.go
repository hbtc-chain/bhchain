...
func TestSysTransferEthToUserHaveEthSuccess(t *testing.T) {
	...
	require.Equal(t, amt, user1CU.GetCoins().AmountOf(symbol))
	require.Equal(t, user1CUEthAddress, user1CU.GetAssetAddress(symbol, 1))
	...
	opCU2 := newTestCU(ck.GetCU(ctx, opCU.GetAddress()))
	...
	opCU3 := newTestCU(ck.GetCU(ctx, ethOPCUAddr))
	...
	item := ik.GetDeposit(ctx, symbol, user1CUAddr, sysTransferTxHash, 0)
...
func TestSysTransferEthToUserHaveUsdtSuccess(t *testing.T) {
	...
	require.Equal(t, user1UsdtAmt, user1CU.GetCoins().AmountOf(usdtSymbol))
	require.Equal(t, user1EthAmt, user1CU.GetCoins().AmountOf(ethSymbol))
	require.Equal(t, user1CUEthAddress, user1CU.GetAssetAddress(ethSymbol, 1))
	require.Equal(t, user1CUEthAddress, user1CU.GetAssetAddress(usdtSymbol, 1))
	...
	//check systransfer deposit item
	item := ik.GetDeposit(ctx, ethSymbol, user1CUAddr, sysTransferTxHash, 0)
	require.Equal(t, sdk.DepositItemStatusConfirmed, item.Status)
	require.Equal(t, sysTransferAmt, item.Amount)
...
func TestSysTransferEthToEthOpCUSuccess(t *testing.T) {
	...
	require.Nil(t, err)
	opCU1 := newTestCU(ck.GetCU(ctx, sdk.CUAddress(ethOPCUAddr1)))
	...
	require.Nil(t, err)
	ik.SaveDeposit(ctx, symbol, ethOPCUAddr1, item)
	...
	opCU2 := newTestCU(ck.NewOpCUWithAddress(ctx, "eth", ethOPCUAddr2))
	...
	require.Nil(t, err)
	ik.SaveDeposit(ctx, symbol, ethOPCUAddr2, item)
	opCU2.AddAssetCoins(sdk.NewCoins(sdk.NewCoin(symbol, opCU2EthAmt)))
	ck.SetCU(ctx, opCU2)

	opCU2 = newTestCU(ck.GetCU(ctx, sdk.CUAddress(ethOPCUAddr2)))
	...
	//Check opCU coins and coinsHold
	opCU1 = newTestCU(ck.GetCU(ctx, opCU1.GetAddress()))
	...
	//check OPCU's coins, coinsHold and status
	opCU1 = newTestCU(ck.GetCU(ctx, ethOPCUAddr1))
	...
	//Check user1 coins and coinsHold
	opCU2 = newTestCU(ck.GetCU(ctx, opCU2.GetAddress()))
	...
	//Check user1 coins and coinsHold
	opCU2 = newTestCU(ck.GetCU(ctx, ethOPCUAddr2))
	...
	//check systransfer deposit item
	item = ik.GetDeposit(ctx, symbol, ethOPCUAddr2, sysTransferTxHash, 0)
	require.Equal(t, sdk.DepositItemStatusConfirmed, item.Status)
	require.Equal(t, sysTransferAmt, item.Amount)
...
func TestSysTransferEthToUsdtOpCUSuccess(t *testing.T) {
	...
	ethOPCUAddr, err := sdk.CUAddressFromBase58("HBCLXBebMwEWaEZYsqJij7xcpBayzJqdrKJP")
	require.Nil(t, err)
	ethOPCU := newTestCU(ck.GetCU(ctx, sdk.CUAddress(ethOPCUAddr)))
	...
	require.Nil(t, err)
	ik.SaveDeposit(ctx, ethSymbol, ethOPCUAddr, item)
	...
	usdtOPCU := newTestCU(ck.NewOpCUWithAddress(ctx, "usdt", usdtOPCUAddr))
	...
	ik.SaveDeposit(ctx, ethSymbol, usdtOPCUAddr, item)
	...
	require.Nil(t, err)
	ik.SaveDeposit(ctx, usdtSymbol, usdtOPCUAddr, item)
	usdtOPCU.AddAssetCoins(sdk.NewCoins(sdk.NewCoin(usdtSymbol, usdtOPCUUsdtAmt)))
	ck.SetCU(ctx, usdtOPCU)

	usdtOPCU = newTestCU(ck.GetCU(ctx, sdk.CUAddress(usdtOPCUAddr)))
	...
	//Check opCU coins and coinsHold
	ethOPCU = newTestCU(ck.GetCU(ctx, ethOPCU.GetAddress()))
	...
	//Check user1 coins and coinsHold
	usdtOPCU = newTestCU(ck.GetCU(ctx, usdtOPCU.GetAddress()))
	...
	usdtOPCU = newTestCU(ck.GetCU(ctx, usdtOPCUAddr))
	...
	//check systransfer deposit item
	item = ik.GetDeposit(ctx, ethSymbol, usdtOPCUAddr, sysTransferTxHash, 0)
	require.Equal(t, sdk.DepositItemStatusConfirmed, item.Status)
	require.Equal(t, sysTransferAmt, item.Amount)
...
func TestSysTransferEthToUserCUError(t *testing.T) {
	...
	//setup eth token info
	ethTokenInfo := tk.GetIBCToken(ctx, sdk.Symbol(chain))
	...
	//setup usdt token info
	usdtTokenInfo := tk.GetIBCToken(ctx, sdk.Symbol(symbol))
	...
	require.Equal(t, amt, user1CU.GetCoins().AmountOf(symbol))
	require.Equal(t, toAddr, user1CU.GetAssetAddress(symbol, 1))
	...
	usdtTokenInfo = tk.GetIBCToken(ctx, sdk.Symbol(symbol))
	usdtTokenInfo.SendEnabled = false
	tk.SetToken(ctx, usdtTokenInfo)
	...
	mockCN.On("ValidAddress", chain, symbol, toAddr).Return(true, toAddr)
	usdtTokenInfo.SendEnabled = true
	usdtTokenInfo.WithdrawalEnabled = false
	tk.SetToken(ctx, usdtTokenInfo)
	result = keeper.SysTransfer(ctx, ethOPCUAddr, user1CUAddr, toAddr, user1SysTransferOrderID, symbol)
	require.Equal(t, sdk.CodeTransactionIsNotEnabled, result.Code)

	//eth's  sendenable is false
	usdtTokenInfo.WithdrawalEnabled = true
	tk.SetToken(ctx, usdtTokenInfo)
	ethTokenInfo = tk.GetIBCToken(ctx, sdk.Symbol(chain))
	ethTokenInfo.SendEnabled = false
	tk.SetToken(ctx, ethTokenInfo)
	result = keeper.SysTransfer(ctx, ethOPCUAddr, user1CUAddr, toAddr, user1SysTransferOrderID, symbol)
	require.Equal(t, sdk.CodeTransactionIsNotEnabled, result.Code)

	//eth's withdrawalenable is false
	ethTokenInfo.SendEnabled = true
	ethTokenInfo.WithdrawalEnabled = false
	tk.SetToken(ctx, ethTokenInfo)
	...
	ethTokenInfo.WithdrawalEnabled = true
	tk.SetToken(ctx, ethTokenInfo)
	...
	result = keeper.SysTransferWaitSign(ctx, "user1SysTransferOrderID", signHash, rawData)
...
func TestSysTransferEthToOPCUError(t *testing.T) {
	...
	item, err := sdk.NewDepositItem("opcu1deposithash", 0, ethOPCUEthAmt, ethOPCUEthAddress, "", sdk.DepositItemStatusConfirmed)
	require.Nil(t, err)
	ik.SaveDeposit(ctx, chain, ethOPCUAddr, item)
	...
	//setup usdtopcu
	usdtOPCUAddr, err := sdk.CUAddressFromBase58("HBCTGvSBNT8oEA4xEqCZGERWkzvpNXCTDGMx")
	usdtOPCU := newTestCU(ck.NewOpCUWithAddress(ctx, symbol, usdtOPCUAddr))
	usdtOPCUEthAddress := "0x81b7E08F65Bdf5648606c89998A9CC8164397647"
	...
	require.Nil(t, err)
	ik.SaveDeposit(ctx, symbol, usdtOPCUAddr, item)
