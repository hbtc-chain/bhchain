...
import (
	...
	gcutils "github.com/hbtc-chain/bhchain/x/gov/client/utils"
...
Example:
$ %s query gov proposals --depositor BHhhzxqNNjw3qx4ZjaR9mnJeLRC1FxPWEyp
$ %s query gov proposals --voter BHhhzxqNNjw3qx4ZjaR9mnJeLRC1FxPWEyp
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var depositorAddr sdk.CUAddress
			var voterAddr sdk.CUAddress
			...
			if len(bechDepositorAddr) != 0 {
				depositorAddr, err := sdk.CUAddressFromBase58(bechDepositorAddr)
				if err != nil {
					return err
				}
			...
			if len(bechVoterAddr) != 0 {
				voterAddr, err := sdk.CUAddressFromBase58(bechVoterAddr)
				if err != nil {
					return err
				}
...
Example:
$ %s query gov vote 1 BHhhzxqNNjw3qx4ZjaR9mnJeLRC1FxPWEyp
`,
				version.ClientName,
			),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			voterAddr, err := sdk.CUAddressFromBase58(args[1])
			if err != nil {
				return err
			}
...
Example:
$ %s query gov deposit 1 BHhhzxqNNjw3qx4ZjaR9mnJeLRC1FxPWEyp
`,
				version.ClientName,
			),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			depositorAddr, err := sdk.CUAddressFromBase58(args[1])
			if err != nil {
				return err
			}
