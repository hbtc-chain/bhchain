...
// content.
type ContentSimulator func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simulation.CU) gov.Content
...
func SimulateSubmittingVotingAndSlashingForProposal(k gov.Keeper, contentSim ContentSimulator) simulation.Operation {
	...
	return func(
		r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simulation.CU,
...
func SimulateTextProposalContent(r *rand.Rand, _ *baseapp.BaseApp, _ sdk.Context, _ []simulation.CU) gov.Content {
...
func simulationCreateMsgSubmitProposal(r *rand.Rand, c gov.Content, s simulation.CU) (msg gov.MsgSubmitProposal, err error) {
	msg = gov.NewMsgSubmitProposal(c, randomDeposit(r), s.Address, 0)
...
func SimulateMsgVote(k gov.Keeper) simulation.Operation {
	return operationSimulateMsgVote(k, simulation.CU{}, 0)
}

// nolint: unparam
func operationSimulateMsgVote(k gov.Keeper, acc simulation.CU, proposalID uint64) simulation.Operation {
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simulation.CU) (
		opMsg simulation.OperationMsg, fOps []simulation.FutureOperation, err error) {

		if acc.Equals(simulation.CU{}) {
			acc = simulation.RandomAcc(r, accs)
		}
...
func randomDeposit(r *rand.Rand) sdk.Coins {
	// TODO Choose based on CU balance and min deposit
