...
func (keeper Keeper) SubmitProposal(ctx sdk.Context, content Content, voteTime uint32) (Proposal, sdk.Error) {
	...
	proposal := NewProposal(content, proposalID, submitTime, submitTime.Add(depositPeriod), voteTime)
...
func (keeper Keeper) GetProposalsFiltered(ctx sdk.Context, voterAddr sdk.CUAddress, depositorAddr sdk.CUAddress, status ProposalStatus, numLatest uint64) []Proposal {
...
func (keeper Keeper) activateVotingPeriod(ctx sdk.Context, proposal Proposal) {
	...
	if proposal.VoteTime == 0 {
		votingPeriod := keeper.GetVotingParams(ctx).VotingPeriod
		proposal.VotingEndTime = proposal.VotingStartTime.Add(votingPeriod)
	} else {
		voteDuration := time.Duration(proposal.VoteTime) * time.Second
		proposal.VotingEndTime = proposal.VotingStartTime.Add(voteDuration)
	}
