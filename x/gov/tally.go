...
func daotally(ctx sdk.Context, keeper Keeper, proposal Proposal) (passes bool, burnDeposits bool, tallyResults TallyResult) {
	results := make(map[VoteOption]sdk.Dec)
	results[OptionYes] = sdk.ZeroDec()
	results[OptionAbstain] = sdk.ZeroDec()
	results[OptionNo] = sdk.ZeroDec()
	results[OptionNoWithVeto] = sdk.ZeroDec()
	totalVote := sdk.ZeroDec()

	keeper.IterateDaoVotes(ctx, proposal.ProposalID, func(vote types.DaoVote) bool {

		results[vote.Option] = results[vote.Option].Add(sdk.NewDecFromInt(vote.VoteAmount.AmountOf(proposal.ProposalToken())))
		totalVote = totalVote.Add(sdk.NewDecFromInt(vote.VoteAmount.AmountOf(proposal.ProposalToken())))
		//back vote token
		_, err := keeper.supplyKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, vote.Voter, vote.VoteAmount)
		if err != nil {
			panic(err)
		}

		keeper.deleteDaoVote(ctx, vote.ProposalID, vote.Voter)
		return false
	})

	tallyParams := keeper.GetTallyParams(ctx)
	tallyResults = NewTallyResultFromMap(results)
	totalcoinsAmt := keeper.supplyKeeper.GetSupply(ctx).GetTotal().AmountOf(proposal.ProposalToken())
	if totalcoinsAmt.LTE(sdk.ZeroInt()) {
		return false, false, tallyResults
	}

	if totalVote.Quo(sdk.NewDecFromInt(totalcoinsAmt)).LTE(tallyParams.Quorum) {
		return false, false, tallyResults
	}

	if results[OptionYes].Quo(totalVote).GT(tallyParams.Threshold) {
		return true, false, tallyResults
	}
