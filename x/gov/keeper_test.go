...
func TestIncrementProposalNumber(t *testing.T) {
	...
	tp := testProposal()
	input.keeper.SubmitProposal(ctx, tp, 0)
	input.keeper.SubmitProposal(ctx, tp, 0)
	input.keeper.SubmitProposal(ctx, tp, 0)
	input.keeper.SubmitProposal(ctx, tp, 0)
	input.keeper.SubmitProposal(ctx, tp, 0)
	proposal6, err := input.keeper.SubmitProposal(ctx, tp, 0)
	require.NoError(t, err)
...
func TestDeposits(t *testing.T) {
	...
	// Check first deposit
	_, err, votingStarted := input.keeper.AddDeposit(ctx, proposalID, input.addrs[0], fourStake)
	require.Nil(t, err)
	...
	deposit, found = input.keeper.GetDeposit(ctx, proposalID, input.addrs[0])
	require.True(t, found)
	require.Equal(t, fourStake.Add(stake25), deposit.Amount)
	require.Equal(t, input.addrs[0], deposit.Depositor)
	proposal, ok = input.keeper.GetProposal(ctx, proposalID)
	require.True(t, ok)
	...
	require.Equal(t, stake25, deposit.Amount)
	proposal, ok = input.keeper.GetProposal(ctx, proposalID)
	require.True(t, ok)
...
func (validProposal) ProposalToken() string    { return sdk.NativeToken }
...
func TestSubmitProposal(t *testing.T) {
	...
	for _, tc := range testCases {
		_, err := input.keeper.SubmitProposal(ctx, tc.content, 0)
