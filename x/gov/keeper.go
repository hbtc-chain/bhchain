...
func NewKeeper(
	...
	if addr := supplyKeeper.GetModuleAddress(types.ModuleName); addr == nil {
		panic(fmt.Sprintf("%s module CustodianUnit has not been set", types.ModuleName))
	}
...
func (keeper Keeper) IterateAllDaoVotes(ctx sdk.Context, cb func(vote types.DaoVote) (stop bool)) {
	store := ctx.KVStore(keeper.storeKey)
	iterator := sdk.KVStorePrefixIterator(store, types.DaoVotesKeyPrefix)

	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		var vote types.DaoVote
		keeper.cdc.MustUnmarshalBinaryLengthPrefixed(iterator.Value(), &vote)
...
// IterateDaoVotes iterates over the all the proposals daovotes and performs a callback function
func (keeper Keeper) IterateDaoVotes(ctx sdk.Context, proposalID uint64, cb func(vote types.DaoVote) (stop bool)) {
	iterator := keeper.GetDaoVotesIterator(ctx, proposalID)
