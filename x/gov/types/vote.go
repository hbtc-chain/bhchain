...
func NewDaoVote(proposalID uint64, voter sdk.CUAddress, voteAmount sdk.Coins, option VoteOption) DaoVote {
	return DaoVote{proposalID, voter, voteAmount, option}
}

func (v DaoVote) String() string {
	return fmt.Sprintf("voter %s voted with voteamount %s, option %s on proposal %d", v.Voter, v.VoteAmount, v.Option, v.ProposalID)
}
...
func (v DaoVotes) String() string {
	...
	out := fmt.Sprintf("Votes for Proposal %d:", v[0].ProposalID)
	for _, vot := range v {
		out += fmt.Sprintf("\n  %s: %s, %s", vot.Voter, vot.VoteAmount, vot.Option)
	}
	return out
}
