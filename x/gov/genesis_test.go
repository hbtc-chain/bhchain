...
func TestEqualProposals(t *testing.T) {
	...
	proposal1, err := input.keeper.SubmitProposal(ctx, proposal, 0)
	require.NoError(t, err)
	proposal2, err := input.keeper.SubmitProposal(ctx, proposal, 0)
	require.NoError(t, err)
...
func TestImportExportQueues(t *testing.T) {
	...
	_, err, votingStarted := input.keeper.AddDeposit(ctx, proposalID2, input.addrs[0], input.keeper.GetDepositParams(ctx).MinDeposit)
	require.NoError(t, err)
	...
	genAccs := input.mApp.CUKeeper.GetAllCUs(ctx)
