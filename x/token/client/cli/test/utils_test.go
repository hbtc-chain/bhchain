...
func TestParseTokenParamsChangeProposalJSON(t *testing.T) {
	...
	assert.Equal(t, sdk.KeySendEnabled, changes[0].Key)
	assert.Equal(t, "true", changes[0].Value)
	assert.Equal(t, sdk.KeyDepositEnabled, changes[1].Key)
	assert.Equal(t, "false", changes[1].Value)
	assert.Equal(t, sdk.KeyWithdrawalEnabled, changes[2].Key)
