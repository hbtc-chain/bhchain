...
func TestCannotUnjailUnlessMeetMinSelfDelegation(t *testing.T) {
	...
	undelegateMsg := staking.NewMsgUndelegate(sdk.CUAddress(addr), addr, unbondAmt)
...
func TestJailedValidatorDelegations(t *testing.T) {
	...
	msgUndelegate := staking.NewMsgUndelegate(sdk.CUAddress(valAddr), valAddr, unbondAmt)
	got = staking.NewHandler(stakingKeeper)(ctx, msgUndelegate)
	require.True(t, got.IsOK(), "expected begin unbonding validator msg to be ok, got: %v", got)

	err := stakingKeeper.CompleteUnbonding(ctx, sdk.CUAddress(valAddr), valAddr)
