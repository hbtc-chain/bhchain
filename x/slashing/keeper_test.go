...
func TestHandleDoubleSign(t *testing.T) {
	...
	ctx = ctx.WithBlockHeight(1)
	valAddrs := make([]sdk.CUAddress, 7)
	for i := 0; i < 7; i++ {
		valAddrs[i] = sdk.CUAddress(addrs[i])
	}
	...
	del, _ := sk.GetDelegation(ctx, sdk.CUAddress(operatorAddr), operatorAddr)
	validator, _ := sk.GetValidator(ctx, operatorAddr)

	totalBond := validator.TokensFromShares(del.GetShares()).TruncateInt()
	msgUnbond := staking.NewMsgUndelegate(sdk.CUAddress(operatorAddr), operatorAddr, sdk.NewCoin(sk.GetParams(ctx).BondDenom, totalBond))
...
func TestHandleAbsentValidator(t *testing.T) {
	...
	sk.SetMigrationFinished(ctx.WithBlockHeight(ctx.BlockHeight() + 1))
	...
	// end block
	staking.EndBlocker(ctx.WithBlockHeight(2505), sk)
...
func TestHandleNewValidator(t *testing.T) {
	...
	ctx = ctx.WithBlockHeight(keeper.SignedBlocksWindow(ctx) + period)
	...
	info, found := keeper.getValidatorSigningInfo(ctx, sdk.ConsAddress(val.Address()))
	require.True(t, found)
	require.Equal(t, keeper.SignedBlocksWindow(ctx)+period, info.StartHeight)
...
func TestHandleAlreadyJailed(t *testing.T) {
	...
	// validator should have been slashed
	resultingTokens := amt.Sub(sdk.TokensFromConsensusPower(10000))
...
func TestValidatorDippingInAndOut(t *testing.T) {
	...
	newAmt := sdk.TokensFromConsensusPower(1010000)
	...
	sk.SetMigrationFinished(ctx.WithBlockHeight(101))
	...
	got = sh(ctx, newTestMsgDelegate(sdk.CUAddress(addrs[2]), addrs[0], delTokens))
	require.True(t, got.IsOK())
	validatorUpdates = staking.EndBlocker(ctx, sk)
	require.Equal(t, 2, len(validatorUpdates))
	validator, _ = sk.GetValidator(ctx, addr)
	...
	staking.EndBlocker(ctx.WithBlockHeight(5505), sk)
	validator, _ = sk.GetValidator(ctx, addr)
	require.Equal(t, sdk.Unbonding, validator.Status)
