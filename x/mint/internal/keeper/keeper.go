...
func NewKeeper(
	...
	// ensure mint module CU is set
	if addr := supplyKeeper.GetModuleAddress(types.ModuleName); addr == nil {
		panic("the mint module CU has not been set")
	}
...
func (k Keeper) TotalBondedToken(ctx sdk.Context) sdk.Int {
	return k.sk.TotalBondedTokens(ctx)
}

func (k Keeper) LastValidatorNumber(ctx sdk.Context) sdk.Int {
	vals := k.sk.GetLastValidators(ctx)
	return sdk.NewInt(int64(len(vals)))
}
...
func (k Keeper) AddCollectedFees(ctx sdk.Context, fees sdk.Coins) sdk.Error {
	_, err := k.supplyKeeper.SendCoinsFromModuleToModule(ctx, types.ModuleName, k.feeCollectorName, fees)
	return err
}
