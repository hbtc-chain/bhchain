...
// GetBondedPool returns the bonded tokens pool's module CustodianUnit
...
// GetNotBondedPool returns the not bonded tokens pool's module CustodianUnit
...
func (k Keeper) bondedTokensToNotBonded(ctx sdk.Context, tokens sdk.Int) sdk.Result {
	coins := sdk.NewCoins(sdk.NewCoin(k.BondDenom(ctx), tokens))
	ret, err := k.supplyKeeper.SendCoinsFromModuleToModule(ctx, types.BondedPoolName, types.NotBondedPoolName, coins)
	if err != nil {
		panic(err)
	}
	return ret
...
func (k Keeper) notBondedTokensToBonded(ctx sdk.Context, tokens sdk.Int) sdk.Result {
	coins := sdk.NewCoins(sdk.NewCoin(k.BondDenom(ctx), tokens))
	ret, err := k.supplyKeeper.SendCoinsFromModuleToModule(ctx, types.NotBondedPoolName, types.BondedPoolName, coins)
	if err != nil {
		panic(err)
	}
	return ret
...
// burnBondedTokens removes coins from the bonded pool module CustodianUnit
...
// burnNotBondedTokens removes coins from the not bonded pool module CustodianUnit
...
func (k Keeper) TotalBondedTokens(ctx sdk.Context) sdk.Int {
	...
	return k.transferKeeper.GetBalance(ctx, bondedPool.GetAddress(), k.BondDenom(ctx))
}
...
func (k Keeper) BondedRatio(ctx sdk.Context) sdk.Dec {
	...
	if stakeSupply.IsPositive() {
		return k.transferKeeper.GetBalance(ctx, bondedPool.GetAddress(), k.BondDenom(ctx)).ToDec().QuoInt(stakeSupply)
	}
	return sdk.ZeroDec()
}
