...
func TestSetValidator(t *testing.T) {
	ctx, _, keeper, _ := CreateTestInput(t, false, 100000)
	...
	validator := types.NewValidator(valAddr, valPubKey, types.Description{}, true)
...
func TestUpdateValidatorByPowerIndex(t *testing.T) {
	...
	bondedPool.SetCoins(sdk.NewCoins(sdk.NewCoin(keeper.BondDenom(ctx), sdk.TokensFromConsensusPower(12340000))))
	notBondedPool.SetCoins(sdk.NewCoins(sdk.NewCoin(keeper.BondDenom(ctx), sdk.TokensFromConsensusPower(100000000))))
	keeper.supplyKeeper.SetModuleAccount(ctx, bondedPool)
	keeper.supplyKeeper.SetModuleAccount(ctx, notBondedPool)

	// add a validator
	validator := types.NewValidator(sdk.ValAddress(addrVals[0]), PKs[0], types.Description{}, true)
	validator, delSharesCreated := validator.AddTokensFromDel(sdk.TokensFromConsensusPower(1000000))
	require.Equal(t, sdk.Unbonded, validator.Status)
	require.Equal(t, sdk.TokensFromConsensusPower(1000000), validator.Tokens)
	...
	require.Equal(t, sdk.TokensFromConsensusPower(500000), burned)
...
func TestUpdateBondedValidatorsDecreaseCliff(t *testing.T) {
	...
	for i := 0; i < len(validators); i++ {
		...
		val := types.NewValidator(sdk.ValAddress(Addrs[i]), PKs[i], types.Description{Moniker: moniker}, true)
		delTokens := sdk.TokensFromConsensusPower(int64((i + 1) * 100000))
	...
	shares := sdk.TokensFromConsensusPower(210000)
...
func TestSlashToZeroPowerRemoved(t *testing.T) {
	// initialize setup
	ctx, _, keeper, _ := CreateTestInput(t, false, 1000000)
	...
	keeper.Slash(ctx, consAddr0, 0, 1000000, sdk.OneDec())
	// apply TM updates
	keeper.ApplyAndReturnValidatorSetUpdates(ctx)
	// validator should be unbonding
	validator, _ = keeper.GetValidator(ctx, sdk.ValAddress(addrVals[0]))
...
func TestValidatorBasics(t *testing.T) {
	...
	powers := []int64{900000, 800000, 700000}
	for i, power := range powers {
		validators[i] = types.NewValidator(sdk.ValAddress(addrVals[i]), PKs[i], types.Description{}, true)
	...
	assert.Equal(t, sdk.TokensFromConsensusPower(900000), validators[0].Tokens)
	assert.Equal(t, sdk.TokensFromConsensusPower(800000), validators[1].Tokens)
	assert.Equal(t, sdk.TokensFromConsensusPower(700000), validators[2].Tokens)
	...
	resVal, found := keeper.GetValidator(ctx, sdk.ValAddress(addrVals[0]))
	require.True(t, found)
	...
	assert.True(sdk.IntEq(t, sdk.TokensFromConsensusPower(900000), validators[0].BondedTokens()))

	// modify a records, save, and retrieve
	validators[0].Status = sdk.Bonded
	validators[0].Tokens = sdk.TokensFromConsensusPower(100000)
	...
	resVal, found = keeper.GetValidator(ctx, sdk.ValAddress(addrVals[1]))
	require.True(t, found)
	assert.True(ValEq(t, validators[1], resVal))
	resVal, found = keeper.GetValidator(ctx, sdk.ValAddress(addrVals[2]))
	require.True(t, found)
	...
	_, found = keeper.GetValidator(ctx, sdk.ValAddress(addrVals[1]))
	require.False(t, found)
}
...
func GetValidatorSortingUnmixed(t *testing.T) {
	...
	amts := []int64{0, 1000000, 10000, 4000000, 2000000}
	n := len(amts)
	...
	assert.Equal(t, sdk.NewInt(4000000), resValidators[0].BondedTokens(), "%v", resValidators)
	assert.Equal(t, sdk.NewInt(2000000), resValidators[1].BondedTokens(), "%v", resValidators)
	assert.Equal(t, sdk.NewInt(1000000), resValidators[2].BondedTokens(), "%v", resValidators)
	assert.Equal(t, sdk.NewInt(10000), resValidators[3].BondedTokens(), "%v", resValidators)
	...
	validators[3].Tokens = sdk.NewInt(5000000)
	...
	validators[3].Tokens = sdk.NewInt(3000000)
	TestingUpdateValidator(keeper, ctx, validators[3], true)
	resValidators = keeper.GetBondedValidatorsByPower(ctx)
	require.Equal(t, len(resValidators), n)
	assert.True(ValEq(t, validators[3], resValidators[0]))
	...
	validators[3].Tokens = sdk.NewInt(2000000)
	...
	validators[3].Tokens = sdk.NewInt(3000000)
	validators[4].Tokens = sdk.NewInt(3000000)
	TestingUpdateValidator(keeper, ctx, validators[3], true)
	resValidators = keeper.GetBondedValidatorsByPower(ctx)
	require.Equal(t, len(resValidators), n)
...
func TestGetValidatorsEdgeCases(t *testing.T) {
	...
	// initialize some validators into the state
	powers := []int64{0, 1000000, 4000000, 4000000}
	var validators [4]types.Validator
	for i, power := range powers {
		moniker := fmt.Sprintf("val#%d", int64(i))
		validators[i] = types.NewValidator(sdk.ValAddress(Addrs[i]), PKs[i], types.Description{Moniker: moniker}, true)
		tokens := sdk.TokensFromConsensusPower(power)
		validators[i], _ = validators[i].AddTokensFromDel(tokens)
		notBondedPool := keeper.GetNotBondedPool(ctx)
	...
	delTokens := sdk.TokensFromConsensusPower(5000000)
	...
	notBondedPool = keeper.GetNotBondedPool(ctx)
	require.NoError(t, notBondedPool.SetCoins(notBondedPool.GetCoins().Add(sdk.NewCoins(sdk.NewCoin(params.BondDenom, sdk.NewInt(2000000))))))
	keeper.supplyKeeper.SetModuleAccount(ctx, notBondedPool)
...
func TestValidatorBondHeight(t *testing.T) {
	...
	// initialize some validators into the state
	var validators [3]types.Validator
	validators[0] = types.NewValidator(sdk.ValAddress(PKs[0].Address().Bytes()), PKs[0], types.Description{}, true)
	validators[1] = types.NewValidator(sdk.ValAddress(Addrs[1]), PKs[1], types.Description{}, true)
	validators[2] = types.NewValidator(sdk.ValAddress(Addrs[2]), PKs[2], types.Description{}, true)

	tokens0 := sdk.TokensFromConsensusPower(2000000)
	tokens1 := sdk.TokensFromConsensusPower(1000000)
	tokens2 := sdk.TokensFromConsensusPower(1000000)
	...
	delTokens := sdk.TokensFromConsensusPower(500000)
...
func TestFullValidatorSetPowerChange(t *testing.T) {
	...
	// initialize some validators into the state
	powers := []int64{0, 1000000, 4000000, 4000000, 2000000}
	var validators [5]types.Validator
	for i, power := range powers {
		validators[i] = types.NewValidator(sdk.ValAddress(Addrs[i]), PKs[i], types.Description{}, true)
		tokens := sdk.TokensFromConsensusPower(power)
	...
	tokens := sdk.TokensFromConsensusPower(6000000)
...
func TestApplyAndReturnValidatorSetUpdatesAllNone(t *testing.T) {
	t.Logf("test for initianizing. should update more than 1/6")
	ctx, _, keeper, _ := CreateTestInput(t, false, 10000000)

	powers := []int64{100000, 200000, 300000}
	var validators [3]types.Validator
	for i, power := range powers {
		valPubKey := PKs[i+1]
		valAddr := sdk.ValAddress(valPubKey.Address().Bytes())

		validators[i] = types.NewValidator(valAddr, valPubKey, types.Description{}, true)
		tokens := sdk.TokensFromConsensusPower(power)
		validators[i], _ = validators[i].AddTokensFromDel(tokens)
	}
	...
	for i := 0; i < 3; i++ {
		validators[i], _ = keeper.GetValidator(ctx, validators[i].OperatorAddress)
		require.Equal(t, validators[i].ABCIValidatorUpdate(), updates[2-i])
	}
}
...
func TestApplyAndReturnValidatorSetUpdatesSingleValueChange(t *testing.T) {
	...
	validators[0].Status = sdk.Bonded
	validators[0].Tokens = sdk.TokensFromConsensusPower(6000000)
	validators[0] = TestingUpdateValidator(keeper, ctx, validators[0], false)
...
func TestApplyAndReturnValidatorSetUpdatesMultipleValueChange(t *testing.T) {
	...
	delTokens1 := sdk.TokensFromConsensusPower(1900000)
	delTokens2 := sdk.TokensFromConsensusPower(800000)
...
func TestApplyAndReturnValidatorSetUpdatesInserted(t *testing.T) {
	...
	powers := []int64{1000000, 2000000, 500000, 1500000, 2500000}
	var validators [5]types.Validator
	for i, power := range powers {
...
func TestApplyAndReturnValidatorSetUpdatesWithCliffValidator(t *testing.T) {
	...
	powers := []int64{100000, 200000, 50000}
	var validators [5]types.Validator
	for i, power := range powers {
...
func TestApplyAndReturnValidatorSetUpdatesPowerDecrease(t *testing.T) {
	...
	// check initial power
	require.Equal(t, int64(1000000), validators[0].GetConsensusPower())
	require.Equal(t, int64(1000000), validators[1].GetConsensusPower())

	// test multiple value change
	//  tendermintUpdate set: {c1, c3} -> {c1', c3'}
	delTokens1 := sdk.TokensFromConsensusPower(200000)
	delTokens2 := sdk.TokensFromConsensusPower(300000)
	...
	// power has changed
	require.Equal(t, int64(800000), validators[0].GetConsensusPower())
	require.Equal(t, int64(700000), validators[1].GetConsensusPower())
...
func TestApplyAndReturnValidatorSetUpdatesNewValidator(t *testing.T) {
	...
	validator = types.NewValidator(valAddr, valPubKey, types.Description{}, true)
	tokens := sdk.TokensFromConsensusPower(5000000)
...
func TestApplyAndReturnValidatorSetUpdatesBondTransition(t *testing.T) {
	...
	powers := []int64{1000000, 2000000, 3000000}
	var validators [3]types.Validator
	...
	for i, power := range powers {
		...
		validators[i] = types.NewValidator(valAddr, valPubKey, types.Description{Moniker: moniker}, true)
		tokens := sdk.TokensFromConsensusPower(power)
		validators[i], _ = validators[i].AddTokensFromDel(tokens)
		keeper.SetValidator(ctx, validators[i])
		keeper.SetValidatorByPowerIndex(ctx, validators[i])
...
func TestApplyAndReturnValidatorSetUpdates_MoreThanOneSixth(t *testing.T) {
	ctx, _, keeper, _ := CreateTestInput(t, false, 10000000)
	const n = 14

	var powers [n]int64
	for i := 0; i < n; i++ {
		powers[i] = int64((i + 1) * 100000)
	}
	var validators [n]types.Validator
	for i, power := range powers {
		...
		tokens := sdk.TokensFromConsensusPower(power)
		validators[i], _ = validators[i].AddTokensFromDel(tokens)
		validators[i] = TestingUpdateValidator(keeper, ctx, validators[i], false)

	}
	require.EqualValues(t, n, len(keeper.ApplyAndReturnValidatorSetUpdates(ctx)))

	oneSixth := sdk.OneSixthCeil(types.DefaultMaxKeyNodes)
	increaseNum := oneSixth + 1
	for i := 0; i < int(increaseNum); i++ {
		delTokens1 := sdk.TokensFromConsensusPower(1000000)
		validators[i], _ = validators[i].AddTokensFromDel(delTokens1)
		validators[i] = TestingUpdateValidator(keeper, ctx, validators[i], false)
	}

	updates := keeper.ApplyAndReturnValidatorSetUpdates(ctx)
	require.EqualValues(t, increaseNum, len(updates))
	for i := 0; i < int(oneSixth); i++ {
		require.Equal(t, validators[int(oneSixth)-i].ABCIValidatorUpdate(), updates[i])
	}
}
...
func TestUpdateValidatorCommission(t *testing.T) {
	...
	val1 := types.NewValidator(sdk.ValAddress(addrVals[0]), PKs[0], types.Description{}, true)
	val2 := types.NewValidator(sdk.ValAddress(addrVals[1]), PKs[1], types.Description{}, true)
