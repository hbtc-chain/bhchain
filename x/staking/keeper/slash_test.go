...
func setupHelper(t *testing.T, power int64) (sdk.Context, Keeper, types.Params) {
	...
	for i := int64(0); i < numVals; i++ {
		validator := types.NewValidator(sdk.ValAddress(addrVals[i]), PKs[i], types.Description{}, true)
...
func TestSlashRedelegation(t *testing.T) {
	ctx, keeper, _ := setupHelper(t, 200000)
	fraction := sdk.NewDecWithPrec(5, 1)
...
func TestSlashWithUnbondingDelegation(t *testing.T) {
	...
	// this may not be the desirable behaviour, ref https://github.com/hbtc-chain/bhchain/issues/1440
	ctx = ctx.WithBlockHeight(13)
	keeper.Slash(ctx, consAddr, 9, 100000, fraction)
	ubd, found = keeper.GetUnbondingDelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]))
	require.True(t, found)
...
func TestSlashWithRedelegation(t *testing.T) {
	ctx, keeper, _ := setupHelper(t, 400000)
	consAddr := sdk.ConsAddress(PKs[0].Address())
	fraction := sdk.NewDecWithPrec(5, 1)
