...
func TestNewQuerier(t *testing.T) {
	...
	bz, errRes = cdc.MarshalJSON(types.NewQueryRedelegationParams(cuaddr, nil, nil))
	require.Nil(t, errRes)
	query.Data = bz
...
func TestQueryValidators(t *testing.T) {
	...
	// Create Validators
	amts := []sdk.Int{sdk.NewInt(90000), sdk.NewInt(80000), sdk.NewInt(70000)}
...
func TestQueryDelegation(t *testing.T) {
	...
	delTokens := sdk.TokensFromConsensusPower(200000)
...
func TestQueryUnbondingDelegation(t *testing.T) {
	...
	_, _, err := keeper.Delegate(ctx, addrAcc1, delAmount, sdk.Unbonded, val1, true)
...
func TestQueryEpoch(t *testing.T) {
	...
	vals := addrVals[2 : 2+4]
	keeper.StartNewEpoch(ctx, vals)

	params := types.QueryEpochParams{
		Index:  0,
		Height: uint64(height + 1),
	}
	bz, errRes := cdc.MarshalJSON(params)
	require.Nil(t, errRes)
	query := abci.RequestQuery{
		Path: "/custom/staking/" + types.QueryEpoch,
		Data: bz,
	}
	res, err := queryEpoch(ctx, query, keeper)
	require.Nil(t, err)
	require.NotNil(t, res)

	var epoch sdk.Epoch
	require.NoError(t, cdc.UnmarshalJSON(res, &epoch))
	fmt.Printf("%v\n", epoch)
