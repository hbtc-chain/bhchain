...
func TestDelegation(t *testing.T) {
	...
	for i, amt := range amts {
		validators[i] = types.NewValidator(sdk.ValAddress(addrVals[i]), PKs[i], types.Description{})
	...
	bond1to1 := types.NewDelegation(addrDels[0], sdk.ValAddress(addrVals[0]), sdk.NewDec(9))

	// check the empty keeper first
	_, found := keeper.GetDelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]))
	require.False(t, found)

	// set and retrieve a record
	keeper.SetDelegation(ctx, bond1to1)
	resBond, found := keeper.GetDelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]))
	...
	bond1to2 := types.NewDelegation(addrDels[0], sdk.ValAddress(addrVals[1]), sdk.NewDec(9))
	bond1to3 := types.NewDelegation(addrDels[0], sdk.ValAddress(addrVals[2]), sdk.NewDec(9))
	bond2to1 := types.NewDelegation(addrDels[1], sdk.ValAddress(addrVals[0]), sdk.NewDec(9))
	bond2to2 := types.NewDelegation(addrDels[1], sdk.ValAddress(addrVals[1]), sdk.NewDec(9))
	bond2to3 := types.NewDelegation(addrDels[1], sdk.ValAddress(addrVals[2]), sdk.NewDec(9))
	...
	for i := 0; i < 3; i++ {
		...
		resVal, err := keeper.GetDelegatorValidator(ctx, addrDels[0], sdk.ValAddress(addrVals[i]))
		require.Nil(t, err)
		require.Equal(t, sdk.ValAddress(addrVals[i]), resVal.GetOperator())

		resVal, err = keeper.GetDelegatorValidator(ctx, addrDels[1], sdk.ValAddress(addrVals[i]))
		require.Nil(t, err)
		require.Equal(t, sdk.ValAddress(addrVals[i]), resVal.GetOperator())

		resDels := keeper.GetValidatorDelegations(ctx, sdk.ValAddress(addrVals[i]))
		require.Len(t, resDels, 2)
	}

	// delete a record
	keeper.RemoveDelegation(ctx, bond2to3)
	_, found = keeper.GetDelegation(ctx, addrDels[1], sdk.ValAddress(addrVals[2]))
	require.False(t, found)
	resBonds = keeper.GetDelegatorDelegations(ctx, addrDels[1], 5)
	require.Equal(t, 2, len(resBonds))
	require.True(t, bond2to1.Equal(resBonds[0]))
	...
	_, found = keeper.GetDelegation(ctx, addrDels[1], sdk.ValAddress(addrVals[0]))
	require.False(t, found)
	_, found = keeper.GetDelegation(ctx, addrDels[1], sdk.ValAddress(addrVals[1]))
	require.False(t, found)
	resBonds = keeper.GetDelegatorDelegations(ctx, addrDels[1], 5)
...
func TestUnbondingDelegation(t *testing.T) {
	...
	resUnbond, found := keeper.GetUnbondingDelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]))
	require.True(t, found)
	...
	_, found = keeper.GetUnbondingDelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]))
	require.False(t, found)
...
func TestUnbondDelegation(t *testing.T) {
	ctx, _, keeper, _, trk := CreateTestInputEx(t, false, 0)

	startTokens := sdk.TokensFromConsensusPower(100000)
	...
	validator := types.NewValidator(sdk.ValAddress(addrVals[0]), PKs[0], types.Description{})
	...
	delegation := types.NewDelegation(addrDels[0], sdk.ValAddress(addrVals[0]), issuedShares)
	keeper.SetDelegation(ctx, delegation)

	bondTokens := sdk.TokensFromConsensusPower(6)
	amount, err := keeper.unbond(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), bondTokens.ToDec())
	require.NoError(t, err)
	require.Equal(t, bondTokens, amount) // shares to be added to an unbonding delegation

	delegation, found := keeper.GetDelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]))
	require.True(t, found)
	validator, found = keeper.GetValidator(ctx, sdk.ValAddress(addrVals[0]))
	require.True(t, found)
...
func TestUnbondingDelegationsMaxEntries(t *testing.T) {
	ctx, _, keeper, _, trk := CreateTestInputEx(t, false, 1)
	startTokens := sdk.TokensFromConsensusPower(100000)
	...
	oldBonded := NewTestCU(ctx, trk, keeper.GetBondedPool(ctx)).GetCoins().AmountOf(bondDenom)
	oldNotBonded := NewTestCU(ctx, trk, keeper.GetNotBondedPool(ctx)).GetCoins().AmountOf(bondDenom)

	// should all pass
	var completionTime time.Time
	for i := uint16(0); i < maxEntries; i++ {
		var err error
		completionTime, err = keeper.Undelegate(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.NewDec(1))
		require.NoError(t, err)
	}
	...
	_, err = keeper.Undelegate(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.NewDec(1))
	require.Error(t, err)
	...
	err = keeper.CompleteUnbonding(ctx, addrDels[0], sdk.ValAddress(addrVals[0]))
	require.NoError(t, err)
...
func TestUndelegateSelfDelegationBelowMinSelfDelegation(t *testing.T) {
	...
	selfDelegation := types.NewDelegation(sdk.CUAddress(sdk.ValAddress(addrVals[0]).Bytes()), sdk.ValAddress(addrVals[0]), issuedShares)
	...
	val0AccAddr := sdk.CUAddress(sdk.ValAddress(addrVals[0]).Bytes())
	_, err = keeper.Undelegate(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), sdk.TokensFromConsensusPower(60000).ToDec())
	require.NoError(t, err)
	...
	validator, found := keeper.GetValidator(ctx, sdk.ValAddress(addrVals[0]))
	require.True(t, found)
	require.Equal(t, sdk.TokensFromConsensusPower(140000), validator.Tokens)
...
func TestUndelegateFromUnbondingValidator(t *testing.T) {
	...
	val0AccAddr := sdk.CUAddress(sdk.ValAddress(addrVals[0]).Bytes())
	_, err = keeper.Undelegate(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), delTokens.ToDec())
	require.NoError(t, err)
	...
	_, err = keeper.Undelegate(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.NewDec(6))
	require.NoError(t, err)
...
func TestUndelegateFromUnbondedValidator(t *testing.T) {
	...
	val0AccAddr := sdk.CUAddress(sdk.ValAddress(addrVals[0]).Bytes())
	selfDelegation := types.NewDelegation(val0AccAddr, sdk.ValAddress(addrVals[0]), issuedShares)
	keeper.SetDelegation(ctx, selfDelegation)
	...
	// unbond the all self-delegation to put validator in unbonding state
	_, err = keeper.Undelegate(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), valTokens.ToDec())
	require.NoError(t, err)
	...
	_, err = keeper.Undelegate(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), unbondTokens.ToDec())
	require.NoError(t, err)

	// unbond rest of the other delegation's shares
	remainingTokens := delTokens.Sub(unbondTokens)
	_, err = keeper.Undelegate(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), remainingTokens.ToDec())
	require.NoError(t, err)
...
func TestUnbondingAllDelegationFromValidator(t *testing.T) {
	...
	_, err = keeper.Undelegate(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), delTokens.ToDec())
	require.NoError(t, err)
	...
	_, found = keeper.GetValidator(ctx, sdk.ValAddress(addrVals[0]))
	require.False(t, found)
}
...
func TestGetRedelegationsFromSrcValidator(t *testing.T) {
	...
	resBond, found := keeper.GetRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]))
	require.True(t, found)

	// get the redelegations one time
	redelegations := keeper.GetRedelegationsFromSrcValidator(ctx, sdk.ValAddress(addrVals[0]))
...
func TestRedelegation(t *testing.T) {
	...
	has := keeper.HasReceivingRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[1]))
	require.False(t, has)

	// set and retrieve a record
	keeper.SetRedelegation(ctx, rd)
	resRed, found := keeper.GetRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]))
	require.True(t, found)
	...
	_, found = keeper.GetRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]))
	require.False(t, found)
...
func TestRedelegateToSameValidator(t *testing.T) {
	...
	_, err = keeper.BeginRedelegation(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[0]), sdk.NewDec(5))
	require.Error(t, err)
...
func TestRedelegationMaxEntries(t *testing.T) {
	ctx, _, keeper, _, trk := CreateTestInputEx(t, false, 0)
	startTokens := sdk.TokensFromConsensusPower(200000)
	...
	validator2 := types.NewValidator(sdk.ValAddress(addrVals[1]), PKs[1], types.Description{})
	...
	for i := uint16(0); i < maxEntries; i++ {
		var err error
		completionTime, err = keeper.BeginRedelegation(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]), sdk.NewDec(1))
		require.NoError(t, err)
	}

	// an additional redelegation should fail due to max entries
	_, err = keeper.BeginRedelegation(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]), sdk.NewDec(1))
	require.Error(t, err)

	// mature redelegations
	ctx = ctx.WithBlockTime(completionTime)
	err = keeper.CompleteRedelegation(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]))
	require.NoError(t, err)
...
func TestRedelegateSelfDelegation(t *testing.T) {
	ctx, _, keeper, _, trk := CreateTestInputEx(t, false, 0)
	ctx = ctx.WithBlockHeight(1)
	startTokens := sdk.TokensFromConsensusPower(300000)
	startCoins := sdk.NewCoins(sdk.NewCoin(keeper.BondDenom(ctx), startTokens))
	...
	_, err = keeper.BeginRedelegation(ctx, val0AccAddr, sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]), delTokens.ToDec())
	require.NoError(t, err)
...
func TestRedelegateFromUnbondingValidator(t *testing.T) {
	...
	// unbond some of the other delegation's shares
	redelegateTokens := sdk.TokensFromConsensusPower(60000)
	_, err = keeper.BeginRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]), redelegateTokens.ToDec())
	require.NoError(t, err)

	// retrieve the unbonding delegation
	ubd, found := keeper.GetRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]))
	require.True(t, found)
	require.Len(t, ubd.Entries, 1)
...
func TestRedelegateFromUnbondedValidator(t *testing.T) {
	...
	redelegationTokens := sdk.TokensFromConsensusPower(60000)
	_, err = keeper.BeginRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]), redelegationTokens.ToDec())
	require.NoError(t, err)

	// no red should have been found
	red, found := keeper.GetRedelegation(ctx, addrDels[0], sdk.ValAddress(addrVals[0]), sdk.ValAddress(addrVals[1]))
