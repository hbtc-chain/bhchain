...
func queryPool(ctx sdk.Context, k Keeper) ([]byte, sdk.Error) {
	...
	pool := types.NewPool(
		k.transferKeeper.GetBalance(ctx, notBondedPool.GetAddress(), bondDenom),
		k.transferKeeper.GetBalance(ctx, bondedPool.GetAddress(), bondDenom),
	)
...
func queryEpoch(ctx sdk.Context, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	var epoch sdk.Epoch
	if params.Index == 0 && params.Height == 0 {
		epoch = k.GetCurrentEpoch(ctx)
	} else if params.Index > 0 {
		var found bool
		epoch, found = k.GetEpoch(ctx, params.Index)
		if !found {
			return nil, sdk.ErrInternal(fmt.Sprintf("epoch %d not found", params.Index))
		}
	} else {
		epoch = k.GetEpochByHeight(ctx, params.Height)
	}
	res, err := codec.MarshalJSONIndent(types.ModuleCdc, epoch)
	if err != nil {
		return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
	}
