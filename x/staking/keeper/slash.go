...
import (
	...
	types "github.com/hbtc-chain/bhchain/x/staking/types"
)
...
func (k Keeper) Slash(ctx sdk.Context, consAddr sdk.ConsAddress, infractionHeight int64, power int64, slashFactor sdk.Dec) {
	...
	k.slashValidator(ctx, validator, infractionHeight, power, slashFactor)
}

func (k Keeper) SlashByOperator(ctx sdk.Context, operator sdk.ValAddress, infractionHeight int64, slashFactor sdk.Dec) {
	logger := k.Logger(ctx)
	...
	k.slashValidator(ctx, validator, infractionHeight, validator.PotentialConsensusPower(), slashFactor)
}

func (k Keeper) slashValidator(ctx sdk.Context, validator types.Validator, infractionHeight int64, power int64, slashFactor sdk.Dec) {
	logger := k.Logger(ctx)
	...
	// ref https://github.com/hbtc-chain/bhchain/issues/1348
	...
	// Burn the slashed tokens from the pool CU and decrease the total supply.
...
// jail a validator
func (k Keeper) JailByOperator(ctx sdk.Context, operator sdk.ValAddress) {
	validator := k.mustGetValidator(ctx, operator)
	k.jailValidator(ctx, validator)
	logger := k.Logger(ctx)
	logger.Info(fmt.Sprintf("validator %s jailed", operator))
	return
}
