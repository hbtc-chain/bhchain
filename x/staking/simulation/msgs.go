...
func SimulateMsgCreateValidator(m custodianunit.CUKeeper, k staking.Keeper) simulation.Operation {
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context,
		...
		amount := m.GetCU(ctx, acc.Address).GetCoins().AmountOf(denom)
...
func SimulateMsgDelegate(m custodianunit.CUKeeper, k staking.Keeper) simulation.Operation {
	handler := staking.NewHandler(k)
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context,
		accs []simulation.CU) (opMsg simulation.OperationMsg, fOps []simulation.FutureOperation, err error) {
		...
		amount := m.GetCU(ctx, delegatorAddress).GetCoins().AmountOf(denom)
		if amount.GT(sdk.ZeroInt()) {
			amount = simulation.RandomAmount(r, amount)
		}
		if amount.Equal(sdk.ZeroInt()) {
...
func SimulateMsgBeginRedelegate(m custodianunit.CUKeeper, k staking.Keeper) simulation.Operation {
	handler := staking.NewHandler(k)
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context,
		accs []simulation.CU) (opMsg simulation.OperationMsg, fOps []simulation.FutureOperation, err error) {
