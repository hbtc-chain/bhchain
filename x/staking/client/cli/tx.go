...
func GetCmdEditValidator(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "edit-validator",
		Short: "edit an existing validator cu",
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := custodianunit.NewTxBuilderFromCLI().WithTxEncoder(custodianunit.DefaultTxEncoder(cdc))
			cliCtx := context.NewCLIContext().WithCodec(cdc)
...
Example:
$ %s tx staking delegate bhvaloper1lh86pkhl6nrvmyezmncwdvhmnr0hj6nlk0caaj 1000stake --from mykey
`,
				version.ClientName,
			),
		),
...
Example:
$ %s tx staking redelegate bhvaloper1lh86pkhl6nrvmyezmncwdvhmnr0hj6nlk0caaj bhvaloper136g7jus2k0x7xv7wfusf5npzl5c3uf2cgrwf7z 100stake --from mykey
`,
				version.ClientName,
			),
		),
...
Example:
$ %s tx staking unbond bhvaloper1lh86pkhl6nrvmyezmncwdvhmnr0hj6nlk0caaj 100stake --from mykey
`,
				version.ClientName,
			),
		),
...
var (
	...
	defaultMinSelfDelegation       = "1" //TODO
)
...
func BuildCreateValidatorMsg(cliCtx context.CLIContext, txBldr custodianunit.TxBuilder) (custodianunit.TxBuilder, sdk.Msg, error) {
