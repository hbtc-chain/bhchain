...
var (
	...
	EpochKey           = []byte{0x61} //prefix for epoch
	EpochByHeightKey   = []byte{0x62} //prefix for epoch indexed by height
	JailedQueueInfoKey = []byte{0x63}
	JailedQueueKey     = []byte{0x64}
)
...
func GetDelegationKey(delAddr sdk.CUAddress, valAddr sdk.ValAddress) []byte {
	return append(GetDelegationsKey(delAddr), valAddr.Bytes()...)
}

// gets the prefix for a delegator for all validators
func GetDelegationsKey(delAddr sdk.CUAddress) []byte {
...
func GetUBDKey(delAddr sdk.CUAddress, valAddr sdk.ValAddress) []byte {
	return append(
...
func GetUBDByValIndexKey(delAddr sdk.CUAddress, valAddr sdk.ValAddress) []byte {
...
// gets the prefix for all unbonding delegations from a delegator
func GetUBDsKey(delAddr sdk.CUAddress) []byte {
...
func GetREDKey(delAddr sdk.CUAddress, valSrcAddr, valDstAddr sdk.ValAddress) []byte {
...
// VALUE: none (key rearrangement used)
func GetREDByValSrcIndexKey(delAddr sdk.CUAddress, valSrcAddr, valDstAddr sdk.ValAddress) []byte {
...
// VALUE: none (key rearrangement used)
func GetREDByValDstIndexKey(delAddr sdk.CUAddress, valSrcAddr, valDstAddr sdk.ValAddress) []byte {
...
func GetREDsKey(delAddr sdk.CUAddress) []byte {
...
func GetREDsByDelToValDstIndexKey(delAddr sdk.CUAddress, valDstAddr sdk.ValAddress) []byte {
	return append(
		GetREDsToValDstIndexKey(valDstAddr),
		delAddr.Bytes()...)
}

func GetEpochKey(index uint64) []byte {
	return append(EpochKey, sdk.Uint64ToBigEndian(index)...)
}

func GetEpochByHeightKey(height uint64) []byte {
	return append(EpochByHeightKey, sdk.Uint64ToBigEndian(height)...)
}

func GetJailedQueueKey(index uint64) []byte {
	return append(JailedQueueKey, sdk.Uint64ToBigEndian(index)...)
}
