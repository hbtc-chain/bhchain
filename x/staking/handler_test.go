...
import (
	...
	keep "github.com/hbtc-chain/bhchain/x/staking/keeper"
...
func TestValidatorByPowerIndex(t *testing.T) {
	...
	bond, found := keeper.GetDelegation(ctx, sdk.CUAddress(validatorAddr), validatorAddr)
	...
	msgUndelegate := NewMsgUndelegate(sdk.CUAddress(validatorAddr), validatorAddr, unbondAmt)
...
func TestDuplicatesMsgCreateValidator(t *testing.T) {
	ctx, _, keeper, _ := keep.CreateTestInput(t, false, 10000000)
	...
	require.Equal(t, addr1, validator.OperatorAddress)
	require.Equal(t, pk1, validator.ConsPubKey)
	require.Equal(t, valTokens, validator.BondedTokens())
	require.Equal(t, valTokens.ToDec(), validator.DelegatorShares)
	...
	require.True(t, found)
	require.Equal(t, sdk.Bonded, validator.Status)
	require.Equal(t, addr2, validator.OperatorAddress)
	require.Equal(t, pk2, validator.ConsPubKey)
	require.True(sdk.IntEq(t, valTokens, validator.Tokens))
	require.True(sdk.DecEq(t, valTokens.ToDec(), validator.DelegatorShares))
	require.Equal(t, Description{}, validator.Description)
}
...
func TestLegacyValidatorDelegations(t *testing.T) {
	ctx, _, keeper, _ := keep.CreateTestInput(t, false, int64(10000000))
	setInstantUnbondPeriod(keeper, ctx)

	bondAmount := sdk.TokensFromConsensusPower(100000)
	...
	msgUndelegate := NewMsgUndelegate(sdk.CUAddress(valAddr), valAddr, unbondAmt)
	...
	msgSelfDelegate := NewTestMsgDelegate(sdk.CUAddress(valAddr), valAddr, bondAmount)
...
func TestIncrementsMsgDelegate(t *testing.T) {
	...
	for i := int64(0); i < 5; i++ {
		...
		//Check that the accounts and the bond CU have the appropriate values
		validator, found := keeper.GetValidator(ctx, validatorAddr)
		require.True(t, found)
		...
		gotDelegatorAcc := accMapper.GetCU(ctx, delegatorAddr).GetCoins().AmountOf(params.BondDenom)
...
func TestEditValidatorDecreaseMinSelfDelegation(t *testing.T) {
	...
	initBond := sdk.TokensFromConsensusPower(1000000)
	ctx, _, keeper, _ := keep.CreateTestInput(t, false, initPower)
	_ = setInstantUnbondPeriod(keeper, ctx)
	...
	msgEditValidator := NewMsgEditValidator(validatorAddr, Description{}, nil, &newMinSelfDelegation, nil)
...
func TestIncrementsMsgUnbond(t *testing.T) {
	...
	// initial balance
	amt1 := accMapper.GetCU(ctx, delegatorAddr).GetCoins().AmountOf(denom)
	...
	amt2 := accMapper.GetCU(ctx, delegatorAddr).GetCoins().AmountOf(denom)
...
func TestMultipleMsgCreateValidator(t *testing.T) {
	...
	for i, validatorAddr := range validatorAddrs {
		...
		// verify that the CU is bonded
		validators := keeper.GetValidators(ctx, 100)
		require.Equal(t, (i + 1), len(validators))

		val := validators[i]
		balanceExpd := initTokens.Sub(valTokens)
		balanceGot := accMapper.GetCU(ctx, delegatorAddrs[i]).GetCoins().AmountOf(params.BondDenom)
		...
		require.Equal(t, balanceExpd, balanceGot, "expected CU to have %d, got %d", balanceExpd, balanceGot)
	}
	...
	for i, validatorAddr := range validatorAddrs {
		...
		gotBalance := accMapper.GetCU(ctx, delegatorAddrs[i]).GetCoins().AmountOf(params.BondDenom)
		require.Equal(t, initTokens, gotBalance, "expected CU to have %d, got %d", initTokens, gotBalance)
	}
}
...
func TestJailValidator(t *testing.T) {
	...
	unbondAmt := sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(10))
	msgUndelegateValidator := NewMsgUndelegate(sdk.CUAddress(validatorAddr), validatorAddr, unbondAmt)
...
func TestValidatorQueue(t *testing.T) {
	...
	// bond a delegator
	delTokens := sdk.TokensFromConsensusPower(100000)
...
func TestUnbondingPeriod(t *testing.T) {
	...
	// begin unbonding
	unbondAmt := sdk.NewCoin(sdk.DefaultBondDenom, sdk.TokensFromConsensusPower(100000))
	msgUndelegate := NewMsgUndelegate(sdk.CUAddress(validatorAddr), validatorAddr, unbondAmt)
	got = handleMsgUndelegate(ctx, msgUndelegate, keeper)
	require.True(t, got.IsOK(), "expected no error")
	...
	_, found := keeper.GetUnbondingDelegation(ctx, sdk.CUAddress(validatorAddr), validatorAddr)
...
func TestRedelegationPeriod(t *testing.T) {
	...
	// initial balance
	amt1 := AccMapper.GetCU(ctx, sdk.CUAddress(validatorAddr)).GetCoins().AmountOf(denom)

	got := handleMsgCreateValidator(ctx, msgCreateValidator, keeper)
	require.True(t, got.IsOK(), "expected no error on runMsgCreateValidator")

	// balance should have been subtracted after creation
	amt2 := AccMapper.GetCU(ctx, sdk.CUAddress(validatorAddr)).GetCoins().AmountOf(denom)
	...
	bal1 := AccMapper.GetCU(ctx, sdk.CUAddress(validatorAddr)).GetCoins()

	// begin redelegate
	redAmt := sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(10))
	msgBeginRedelegate := NewMsgBeginRedelegate(sdk.CUAddress(validatorAddr), validatorAddr, validatorAddr2, redAmt)
	got = handleMsgBeginRedelegate(ctx, msgBeginRedelegate, keeper)
	require.True(t, got.IsOK(), "expected no error, %v", got)

	// origin CU should not lose tokens as with a regular delegation
	bal2 := AccMapper.GetCU(ctx, sdk.CUAddress(validatorAddr)).GetCoins()
	...
	EndBlocker(ctx, keeper)
	_, found := keeper.GetRedelegation(ctx, sdk.CUAddress(validatorAddr), validatorAddr, validatorAddr2)
	require.True(t, found, "should not have unbonded")
...
func TestTransitiveRedelegation(t *testing.T) {
	...
	msgBeginRedelegate = NewMsgBeginRedelegate(sdk.CUAddress(validatorAddr), validatorAddr2, validatorAddr3, redAmt)
	got = handleMsgBeginRedelegate(ctx, msgBeginRedelegate, keeper)
...
func TestMultipleRedelegationAtSameTime(t *testing.T) {
	...
	// begin a redelegate
	selfDelAddr := sdk.CUAddress(valAddr) // (the validator is it's own delegator)
...
func TestUnbondingWhenExcessValidators(t *testing.T) {
	...
	valTokens1 := sdk.TokensFromConsensusPower(500000)
	...
	valTokens2 := sdk.TokensFromConsensusPower(300000)
	...
	valTokens3 := sdk.TokensFromConsensusPower(100000)
	...
	msgUndelegate := NewMsgUndelegate(sdk.CUAddress(validatorAddr2), validatorAddr2, unbondAmt)
	got = handleMsgUndelegate(ctx, msgUndelegate, keeper)
...
func TestBondUnbondRedelegateSlashTwice(t *testing.T) {
	...
	unbondAmt := sdk.NewCoin(sdk.DefaultBondDenom, sdk.TokensFromConsensusPower(40000))
	msgUndelegate := NewMsgUndelegate(del, valA, unbondAmt)
	got = handleMsgUndelegate(ctx, msgUndelegate, keeper)
	require.True(t, got.IsOK(), "expected no error on runMsgUndelegate")

	// begin redelegate 6 stake
	redAmt := sdk.NewCoin(sdk.DefaultBondDenom, sdk.TokensFromConsensusPower(60000))
	...
	keeper.Slash(ctx, consAddr0, 0, 200000, sdk.NewDecWithPrec(5, 1))
	...
	keeper.Slash(ctx, consAddr0, 2, 100000, sdk.NewDecWithPrec(5, 1))
	...
	// end blocker
	EndBlocker(ctx.WithBlockHeight(5), keeper)
...
func TestInvalidCoinDenom(t *testing.T) {
	...
	valTokens := sdk.TokensFromConsensusPower(1000000)
	...
	msgCreate := types.NewMsgCreateValidator(valA, keep.PKs[0], invalidCoin, Description{}, commission, sdk.OneInt(), true)
	got := handleMsgCreateValidator(ctx, msgCreate, keeper)
	require.False(t, got.IsOK())
	msgCreate = types.NewMsgCreateValidator(valA, keep.PKs[0], validCoin, Description{}, commission, sdk.OneInt(), true)
	got = handleMsgCreateValidator(ctx, msgCreate, keeper)
	require.True(t, got.IsOK())
	msgCreate = types.NewMsgCreateValidator(valB, keep.PKs[1], validCoin, Description{}, commission, sdk.OneInt(), true)
	got = handleMsgCreateValidator(ctx, msgCreate, keeper)
	require.True(t, got.IsOK())
...
var defaultMinSelfDelegation = sdk.NewIntWithDecimal(50, 22)

func TestValidatorChange(t *testing.T) {
	ctx, cuKeeper, keeper, _ := keep.CreateTestInput(t, false, 100000000)

	for _, addr := range keep.Addrs {
		require.Equal(t, sdk.TokensFromConsensusPower(100000000), cuKeeper.GetCU(ctx, addr).GetCoins().AmountOf(sdk.DefaultBondDenom))
	}

	params := keeper.GetParams(ctx)
	params.MaxValidators = 7
	params.MaxKeyNodes = 4
	params.MinKeyNodeDelegation = defaultMinSelfDelegation
	keeper.SetParams(ctx, params)

	for i := 0; i < 7; i++ {
		valTokens := defaultMinSelfDelegation.MulRaw(int64(i + 1))
		msgCreateValidator1 := NewTestMsgCreateValidatorWithMinSelfDelegation(sdk.ValAddress(keep.Addrs[i]), keep.PKs[i], valTokens, defaultMinSelfDelegation, true)
		got := handleMsgCreateValidator(ctx, msgCreateValidator1, keeper)
		require.True(t, got.IsOK(), "%v", got)
	}

	updates := EndBlocker(ctx, keeper)
	require.Equal(t, 7, len(updates))

	ctx = ctx.WithBlockHeight(ctx.BlockHeight() + 1)

	for i := 0; i < 7; i++ {
		validator, found := keeper.GetValidator(ctx, sdk.ValAddress(keep.Addrs[i]))
		require.True(t, found)
		require.Equal(t, sdk.Bonded, validator.Status)
		require.Equal(t, sdk.ValAddress(keep.Addrs[i]), validator.OperatorAddress)
		require.Equal(t, keep.PKs[i], validator.ConsPubKey)
		require.Equal(t, defaultMinSelfDelegation.MulRaw(int64(i+1)), validator.BondedTokens())
		require.Equal(t, defaultMinSelfDelegation.MulRaw(int64(i+1)).ToDec(), validator.DelegatorShares)
		require.Equal(t, Description{}, validator.Description)
		isKeyNode, _ := keeper.IsActiveKeyNode(ctx, sdk.CUAddress(validator.OperatorAddress))
		require.Equal(t, i > 2, isKeyNode, i)

	}

	require.Equal(t, int(7), len(keeper.GetAllDelegations(ctx)))

	// no updates because not enough  money staked
	ctx = ctx.WithBlockHeight(5)
	msgDelegate := NewTestMsgDelegate(sdk.CUAddress(keep.Addrs[2]), sdk.ValAddress(keep.Addrs[1]), sdk.TokensFromConsensusPower(1).SubRaw(1))
	got := handleMsgDelegate(ctx, msgDelegate, keeper)
	require.True(t, got.IsOK(), "%v", got)

	validator, found := keeper.GetValidator(ctx, sdk.ValAddress(keep.Addrs[1]))
	require.True(t, found)
	require.Equal(t, defaultMinSelfDelegation.MulRaw(2).Add(sdk.TokensFromConsensusPower(1).SubRaw(1)), validator.BondedTokens())

	updates = EndBlocker(ctx, keeper)
	require.Equal(t, 0, len(updates))

	keeper.SetMigrationFinished(ctx.WithBlockHeight(ctx.BlockHeight() + 1))

	// 1 more token staked @height =3
	ctx = ctx.WithBlockHeight(10)
	msgDelegate = NewTestMsgDelegate(sdk.CUAddress(keep.Addrs[2]), sdk.ValAddress(keep.Addrs[1]), sdk.NewInt(1))
	got = handleMsgDelegate(ctx, msgDelegate, keeper)
	require.True(t, got.IsOK(), "%v", got)

	validator, found = keeper.GetValidator(ctx, sdk.ValAddress(keep.Addrs[1]))
	require.True(t, found)
	require.Equal(t, defaultMinSelfDelegation.MulRaw(2).Add(sdk.TokensFromConsensusPower(1)), validator.BondedTokens())

	updates = EndBlocker(ctx, keeper)
	require.Equal(t, 1, len(updates))

	//a validator 10 is created @height=4, validator will be unbounded
	ctx = ctx.WithBlockHeight(15)
	valTokens := defaultMinSelfDelegation.MulRaw(10)
	msgCreateValidator10 := NewTestMsgCreateValidatorWithMinSelfDelegation(sdk.ValAddress(keep.Addrs[10]), keep.PKs[10], valTokens, defaultMinSelfDelegation, true)
	got = handleMsgCreateValidator(ctx, msgCreateValidator10, keeper)
	require.False(t, got.IsOK(), "%v", got)

	msgCreateValidator10 = NewTestMsgCreateValidatorWithMinSelfDelegation(sdk.ValAddress(keep.Addrs[10]), keep.PKs[10], valTokens, defaultMinSelfDelegation, false)
	got = handleMsgCreateValidator(ctx, msgCreateValidator10, keeper)
	require.True(t, got.IsOK(), "%v", got)
	...
	validator, found = keeper.GetValidator(ctx, sdk.ValAddress(keep.Addrs[10]))
	require.True(t, found)
	require.Equal(t, sdk.Bonded, validator.Status)
	require.Equal(t, sdk.ValAddress(keep.Addrs[10]), validator.OperatorAddress)
	require.Equal(t, keep.PKs[10], validator.ConsPubKey)
	require.Equal(t, valTokens, validator.BondedTokens())
	require.Equal(t, valTokens.ToDec(), validator.DelegatorShares)

	validator, found = keeper.GetValidator(ctx, sdk.ValAddress(keep.Addrs[0]))
	require.True(t, found)
	require.Equal(t, sdk.Unbonding, validator.Status)
	require.Equal(t, sdk.ValAddress(keep.Addrs[0]), validator.OperatorAddress)
	require.Equal(t, keep.PKs[0], validator.ConsPubKey)
	require.Equal(t, sdk.ZeroInt(), validator.BondedTokens())
	require.Equal(t, defaultMinSelfDelegation.ToDec(), validator.DelegatorShares)
}
