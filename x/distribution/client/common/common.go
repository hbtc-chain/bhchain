...
func QueryDelegatorTotalRewards(cliCtx context.CLIContext, queryRoute, delAddr string) ([]byte, error) {
	delegatorAddr, err := sdk.CUAddressFromBase58(delAddr)
	if err != nil {
		return nil, err
	}
...
func QueryDelegatorValidators(cliCtx context.CLIContext, queryRoute string, delegatorAddr sdk.CUAddress) ([]byte, error) {
	res, _, err := cliCtx.QueryWithData(
...
func WithdrawAllDelegatorRewards(cliCtx context.CLIContext, queryRoute string, delegatorAddr sdk.CUAddress) ([]sdk.Msg, error) {
...
func WithdrawValidatorRewardsAndCommission(validatorAddr sdk.ValAddress) ([]sdk.Msg, error) {
	...
	rewardMsg := types.NewMsgWithdrawDelegatorReward(sdk.CUAddress(validatorAddr.Bytes()), validatorAddr)
