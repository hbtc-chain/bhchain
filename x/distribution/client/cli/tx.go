...
import (
	...
	"github.com/hbtc-chain/bhchain/x/distribution/client/common"
...
type generateOrBroadcastFunc func(context.CLIContext, custodianunit.TxBuilder, []sdk.Msg) error

func splitAndApply(
	generateOrBroadcast generateOrBroadcastFunc,
	cliCtx context.CLIContext,
	txBldr custodianunit.TxBuilder,
	msgs []sdk.Msg,
	chunkSize int,
) error {
...
Example:
$ %s tx distr withdraw-rewards bhvaloper1lh86pkhl6nrvmyezmncwdvhmnr0hj6nlk0caaj --from mykey
$ %s tx distr withdraw-rewards bhvaloper1lh86pkhl6nrvmyezmncwdvhmnr0hj6nlk0caaj --from mykey --commission
`,
...
Example:
$ %s tx set-withdraw-addr BHhhzxqNNjw3qx4ZjaR9mnJeLRC1FxPWEyp --from mykey
`,
				version.ClientName,
			),
		),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			delAddr := cliCtx.GetFromAddress()
			withdrawAddr, err := sdk.CUAddressFromBase58(args[0])
			if err != nil {
				return err
			}
...

{
  ...
  "recipient": "BHhhzxqNNjw3qx4ZjaR9mnJeLRC1FxPWEyp",
  "amount": [
    {
      "denom": "stake",
      "amount": "10000"
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := gov.NewMsgSubmitProposal(content, proposal.Deposit, from, proposal.VoteTime)
