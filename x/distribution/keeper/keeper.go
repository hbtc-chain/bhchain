...
func (k Keeper) WithdrawDelegationRewards(ctx sdk.Context, delAddr sdk.CUAddress, valAddr sdk.ValAddress) (sdk.Result, sdk.Coins, sdk.Error) {
	val := k.stakingKeeper.Validator(ctx, valAddr)
	if val == nil {
		return sdk.Result{}, nil, types.ErrNoValidatorDistInfo(k.codespace)
	}

	del := k.stakingKeeper.Delegation(ctx, delAddr, valAddr)
	if del == nil {
		return sdk.Result{}, nil, types.ErrNoDelegationDistInfo(k.codespace)
	}

	// withdraw rewards
	result, rewards, err := k.withdrawDelegationRewards(ctx, val, del)
	if err != nil {
...
func (k Keeper) WithdrawValidatorCommission(ctx sdk.Context, valAddr sdk.ValAddress) (sdk.Result, sdk.Coins, sdk.Error) {
	// fetch validator accumulated commission
	accumCommission := k.GetValidatorAccumulatedCommission(ctx, valAddr)
	if accumCommission.IsZero() {
		return sdk.Result{}, nil, types.ErrNoValidatorCommission(k.codespace)
	}
	...
	if !commission.IsZero() {
		accAddr := sdk.CUAddress(valAddr)
		withdrawAddr := k.GetDelegatorWithdrawAddr(ctx, accAddr)
		var err sdk.Error
		result, err = k.supplyKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, withdrawAddr, commission)
		if err != nil {
			return result, nil, err
		}
	}
