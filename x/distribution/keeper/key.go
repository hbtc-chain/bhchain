...
//
// - 0x03<accAddr_Bytes>: sdk.CUAddress
//
...
var (
	...
	ParamStoreKeyKeyNodeReward       = []byte("keynodereward")
)
...
func GetDelegatorWithdrawInfoAddress(key []byte) (delAddr sdk.CUAddress) {
	addr := key[1:]
	if len(addr) != sdk.AddrLen {
		panic("unexpected key length")
	}
...
func GetDelegatorStartingInfoAddresses(key []byte) (valAddr sdk.ValAddress, delAddr sdk.CUAddress) {
...
func GetDelegatorWithdrawAddrKey(delAddr sdk.CUAddress) []byte {
	return append(DelegatorWithdrawAddrPrefix, delAddr.Bytes()...)
}

// gets the key for a delegator's starting info
func GetDelegatorStartingInfoKey(v sdk.ValAddress, d sdk.CUAddress) []byte {
