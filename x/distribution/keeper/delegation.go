...
func (k Keeper) initializeDelegation(ctx sdk.Context, val sdk.ValAddress, del sdk.CUAddress) {
...
func (k Keeper) calculateDelegationRewards(ctx sdk.Context, val exported.ValidatorI, del exported.DelegationI, endingPeriod uint64) (rewards sdk.DecCoins) {
	...
	// Slashes this block happened after reward allocation, but we have to CustodianUnit
	...
	if stake.GT(currentStake) {
		// CustodianUnit for rounding inconsistencies between:
		//
...
func (k Keeper) withdrawDelegationRewards(ctx sdk.Context, val exported.ValidatorI, del exported.DelegationI) (sdk.Result, sdk.Coins, sdk.Error) {
	...
	result := sdk.Result{}
	// add coins to user CustodianUnit
	if !coins.IsZero() {
		withdrawAddr := k.GetDelegatorWithdrawAddr(ctx, del.GetDelegatorAddr())
		var err sdk.Error
		result, err = k.supplyKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, withdrawAddr, coins)
		if err != nil {
