...
import (
	...
	"github.com/hbtc-chain/bhchain/store/prefix"
...
func (k Keeper) GetEvidenceHandler(evidenceRoute string) (types.Handler, error) {
	if !k.router.HasRoute(evidenceRoute) {
		return nil, types.ErrNoEvidenceHandlerExists(evidenceRoute)
	}
...
func (k Keeper) SubmitEvidence(ctx sdk.Context, evidence exported.Evidence) sdk.Error {
	if _, ok := k.GetEvidence(ctx, evidence.Hash()); ok {
		return types.ErrEvidenceExists(evidence.Hash().String())
	}
	if !k.router.HasRoute(evidence.Route()) {
		return types.ErrNoEvidenceHandlerExists(evidence.Route())
	}

	handler := k.router.GetRoute(evidence.Route())
	if err := handler(ctx, evidence); err != nil {
		return types.ErrInvalidEvidence(err.Error())
	}
...
func (k Keeper) SetEvidence(ctx sdk.Context, evidence exported.Evidence) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.EvidencePrefixKey)
...
func (k Keeper) MarshalEvidence(evidenceI exported.Evidence) ([]byte, error) {
	return k.cdc.MarshalBinaryBare(evidenceI)
}
...
func (k Keeper) UnmarshalEvidence(bz []byte) (exported.Evidence, error) {
	...
	if err := k.cdc.UnmarshalBinaryBare(bz, &evi); err != nil {
		return nil, err
	}
