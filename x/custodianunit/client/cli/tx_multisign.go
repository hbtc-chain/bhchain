...
import (
	...
	crkeys "github.com/hbtc-chain/bhchain/crypto/keys"
...
Thus CU number or sequence number lookups will not be performed and it is
...
func makeMultiSignCmd(cdc *codec.Codec) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) (err error) {
		...
		if !viper.GetBool(flagOffline) {
			seq, err := types.NewCURetriever(cliCtx).GetSequence(multisigInfo.GetAddress())
			if err != nil {
				return err
			}
		...
		for i := 2; i < len(args); i++ {
			...
			sigBytes := types.StdSignBytes(
				txBldr.ChainID(), txBldr.Sequence(),
