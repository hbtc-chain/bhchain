...
As a result, the CU and sequence number queries will not be performed and
it is required to set such parameters manually. Note, invalid values will cause
the transaction to fail.

The --multisig=<multisig_key> flag generates a signature on behalf of a multisig CU
...
`,
	...
	cmd.Flags().String(
		flagMultisig, "",
		"Address of the multisig CU on behalf of which the transaction shall be signed",
	)
	cmd.Flags().Bool(
		flagAppend, true,
	...
	cmd.Flags().Bool(
		flagOffline, false,
		"Offline mode; Do not query a full node. --CU and --sequence options would be ignored if offline is set",
	)
...
func preSignCmd(cmd *cobra.Command, _ []string) {
	// Conditionally mark the CU and sequence numbers required as no RPC
	// query will be done.
	if viper.GetBool(flagOffline) {
		cmd.MarkFlagRequired(flags.FlagCUNumber)
...
func makeSignCmd(cdc *codec.Codec) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		if multisigAddrStr != "" {
			var multisigAddr sdk.CUAddress

			multisigAddr, err = sdk.CUAddressFromBase58(multisigAddrStr)
			if err != nil {
				return err
			}
...
func printAndValidateSigs(
	...
	for i, sig := range sigs {
		sigAddr := sdk.CUAddress(sig.Address())
		sigSanity := "OK"
		...
		// reach out to a full node to query CUs.
		if !offline && success {
			cu, err := types.NewCURetriever(cliCtx).GetCU(sigAddr)
			if err != nil {
				fmt.Printf("failed to get CU: %s\n", sigAddr)
				return false
			}

			sigBytes := types.StdSignBytes(
				chainID, cu.GetSequence(),
		...
		if ok {
			...
			for i := 0; i < multiSig.BitArray.Size(); i++ {
				if multiSig.BitArray.GetIndex(i) {
					addr := sdk.CUAddress(multiPK.PubKeys[i].Address().Bytes())
