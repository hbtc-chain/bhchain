...
import (
	...
	genutilrest "github.com/hbtc-chain/bhchain/x/genutil/client/rest"
)

// query CUREST Handler
func QueryCURequestHandlerFn(storeName string, cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		CU, height, err := cuGetter.GetCUWithHeight(addr)
		if err != nil {
			// TODO: Handle more appropriately based on the error type.
			// Ref: https://github.com/hbtc-chain/bhchain/issues/4923
			if err := cuGetter.EnsureExists(addr); err != nil {
				cliCtx = cliCtx.WithHeight(height)
				rest.PostProcessResponse(w, cliCtx, types.BaseCU{})
				return
			}
		...
		cliCtx = cliCtx.WithHeight(height)
		rest.PostProcessResponse(w, cliCtx, CU)
	}
}

func QueryPendingDepositRequestHandlerFn(storeName string, cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		vars := mux.Vars(r)
		addr, err := sdk.CUAddressFromBase58(vars["address"])
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
			return
		}
		...
		params := types.NewQueryPendingDepositListParams(addr)
		...
		res, height, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryPendingDepositList), bz)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
			return
		}
