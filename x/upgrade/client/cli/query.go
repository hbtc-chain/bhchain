...
func GetPlanCmd(cdc *codec.Codec) *cobra.Command {
...
func GetAppliedHeightCmd(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var applied int64
			err = cdc.UnmarshalJSON(res, &applied)
			if err != nil {
				return err
			}
			if applied == 0 {
				return errors.New("no applied found")
			}
